...
export const NemtProvider = ({
  ...
  const setState = useCallback(value => {
    const currentPersistedState = buildClientState(persistedStateRef.current ?? createInitialState());
    const currentSelectedDriverId = selectedDriverIdRef.current || null;
    ...
    const persistedStateChanged = JSON.stringify(currentPersistedState) !== JSON.stringify(nextPersistedState);
    const selectedDriverChanged = currentSelectedDriverId !== nextSelectedDriverId;

    if (!persistedStateChanged && !selectedDriverChanged) return;

    persistedStateRef.current = nextPersistedState;
    selectedDriverIdRef.current = nextSelectedDriverId;
    if (selectedDriverChanged) setSelectedDriverIdState(nextSelectedDriverId);
    if (persistedStateChanged) setPersistedState(nextPersistedState);
  }, [setPersistedState]);
