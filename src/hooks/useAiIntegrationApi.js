...
const useAiIntegrationApi = () => {
  ...
  const refresh = async () => {
    ...
    setError('');
    try {
      const response = await fetch('/api/integrations/ai', { cache: 'no-store' });
      const payload = await response.json();
      if (!response.ok) throw new Error(payload?.error || 'Unable to load AI integration');
      setData(payload);
    } catch (fetchError) {
      setError(fetchError.message || 'Unable to load AI integration');
    } finally {
      setLoading(false);
    }
  };
  ...
  const saveData = async nextState => {
    ...
    try {
      ...
      const message = saveError.message || 'Unable to save AI integration';
      setError(message);
      throw saveError;
    } finally {
      setSaving(false);
