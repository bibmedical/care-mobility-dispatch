...
const MapScreenWorkspace = () => {
  ...
  const sharedMapPoints = useMemo(() => {
    if (showTripSelection || showRouteSelection) return sharedTripPoints.map(point => point.position);
      ...
      <div style={panelStyle}>
        ...
        <div style={mapSurfaceStyle}>
          <MapContainer center={DEFAULT_CENTER} zoom={DEFAULT_ZOOM} zoomControl={false} scrollWheelZoom dragging doubleClickZoom touchZoom boxZoom keyboard preferCanvas style={{ height: '100%', width: '100%' }}>
            ...
            {showSharedSelection && showDriverSelection ? sharedDriverPoints.map(driver => <CircleMarker key={`driver-${driver.id}`} center={driver.position} radius={11} pathOptions={{ color: '#f59e0b', fillColor: '#fbbf24', fillOpacity: 0.9, weight: 3 }}>
                <Popup>
