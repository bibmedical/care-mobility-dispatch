...
(async () => {
  ...
  const snapshotRow = await client.query(
    "select data from dispatch_restore_points where id = $1 limit 1",
    [latestGood.id]
  );
  const snapshot = snapshotRow.rows[0]?.data || {};
  const snapshotTrips = Array.isArray(snapshot.trips) ? snapshot.trips : [];

  const currentTripsRow = await client.query(
    "select id, data from dispatch_trips where service_date = $1",
    [day]
  );
  ...
  for (const trip of snapshotTrips) {
    ...
    const shouldRecover = protectedStatuses.has(snapStatus) || !!snapDriverId || !!snapRouteId;
    ...
    const needsRecover = (!curDriverId && !!snapDriverId) || (!curRouteId && !!snapRouteId) || (curStatus === 'Unassigned' && protectedStatuses.has(snapStatus));
    if (!needsRecover) continue;

    const nextData = {
      ...current,
      driverId: snapDriverId || current.driverId || '',
      secondaryDriverId: String(trip?.secondaryDriverId || '').trim() || current.secondaryDriverId || '',
    ...
    await client.query(
      "update dispatch_trips set data = $1::jsonb, updated_at = now() where id = $2",
      [JSON.stringify(nextData), tripId]
    );
    updates += 1;
  }
